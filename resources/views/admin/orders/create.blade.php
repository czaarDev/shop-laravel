    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.orders.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control mask_money {{ $errors->has('amount') ? 'is-invalid' : '' }}" type="text" name="amount" id="amount" value="{{ old('amount', '') }}" step="0.01" required>
            ...
            <div class="form-group">
                <label class="required" for="order_items">{{ trans('cruds.order.fields.order_item') }}</label>
                <div style="padding-bottom: 4px">
                    <span class="btn btn-info btn-xs select-all" style="border-radius: 0">{{ trans('global.select_all') }}</span>
                    <span class="btn btn-info btn-xs deselect-all" style="border-radius: 0">{{ trans('global.deselect_all') }}</span>
                </div>
                <select class="form-control select2 {{ $errors->has('order_items') ? 'is-invalid' : '' }}" name="order_items[]" id="order_items" multiple required>
                    @foreach($order_items as $id => $order_item)
                        <option value="{{ $id }}" {{ in_array($id, old('order_items', [])) ? 'selected' : '' }}>{{ $order_item }}</option>
                    @endforeach
                </select>
                @if($errors->has('order_items'))
                    <div class="invalid-feedback">
                        {{ $errors->first('order_items') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.order.fields.order_item_helper') }}</span>
            </div>
            <div class="form-group">
