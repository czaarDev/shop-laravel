        ...
        <div class="card-body">
            ...
            >
                ...
                <div class="form-group">
                    ...
                    @foreach(\App\Enums\TypeBannerEnum::getDescriptions() as $type)
                        <div class="form-check {{ $errors->has('type') ? 'is-invalid' : '' }}">
                            <input class="form-check-input"
                                   ...
                                   name="type"
                                   value="{{ $type['value'] }}"
                                   {{ old('type', $banner->type ?? \App\Enums\TypeBannerEnum::HIGHLIGHT->name) === (string) $type['value'] ? 'checked' : '' }}
                                   required
                            />
                            <label class="form-check-label" for="type_{{ $type['value'] }}">{{ $type['name'] }}</label>
                        </div>
                    @endforeach
                ...
                <div class="form-group">
                    ...
                    <input class="form-control date {{ $errors->has('start_at') ? 'is-invalid' : '' }}" type="text" name="start_at" id="start_at" value="{{ old('start_at', $banner->start_at ?? "") }}" required>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control date {{ $errors->has('end_at') ? 'is-invalid' : '' }}" type="text" name="end_at" id="end_at" value="{{ old('end_at', $banner->end_at ?? "") }}" required>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('position') ? 'is-invalid' : '' }}" type="number" name="position" id="position" value="{{ old('position', $banner->position ?? "") }}" step="1" required>
                ...
                <div class="form-group">
                    ...
                    @foreach(App\Models\Banner::STATUS_RADIO as $key => $label)
                        <div class="form-check {{ $errors->has('status') ? 'is-invalid' : '' }}">
                            <input class="form-check-input" type="radio" id="status_{{ $key }}" name="status" value="{{ $key }}" {{ old('status', $banner->status ?? \App\Enums\StatusEmum::ACTIVE->name) === (string) $key ? 'checked' : '' }} required>
